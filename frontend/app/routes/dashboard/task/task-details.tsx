...
const TaskDetails = () => {
  ...
  // -------------------------------
  // ⭐ FIXED TYPE SAFE DELETE HANDLER
  // -------------------------------
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(url, {
        method: "DELETE",
        credentials: "include", // important so auth works
        headers: {
      ...
      navigate(`/dashboard/${workspaceId}/projects/${projectId}`);
    } catch (err) {
  ...
  const normalizedAssignees =
    task.assignees
      ?.map((a: any) =>
        typeof a === "string"
          ? project.members.find((pm) => pm.user._id === a)?.user
          : a
      )
      .filter(Boolean) || [];
